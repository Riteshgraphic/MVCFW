...
class subjects(ctk.CTkScrollableFrame):
    def __init__(self, master, height, width, data, **kwags):
        ...
        label=ctk.CTkButton(upperbar,text="Subjects", corner_radius=0)
        ...
        lbh=ctk.CTkLabel(headerframe, text="Add Subjects")
        ...
        lbid=ctk.CTkLabel(sec1, text="Subject Name :", width=100)
        ...
        lbh=ctk.CTkLabel(headerframe, text="All Subjects")
        lbh.pack(padx=0, pady=0, fill="y", side="left")
        
        line=ctk.CTkFrame(self, height=1.5, width=width, fg_color="#DADADA")
        line.pack(padx=0, pady=0, fill="y")
        
        headings2=("id", "Subject")
        
        tdata2 = []
        if data.get("data"):
            for info in data.get("data"):
                tdata2.append([info.id, info.name])
